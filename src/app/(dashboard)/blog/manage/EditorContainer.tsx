...
import { ArrowLeft, Save, Globe, Lock, Settings, Image as ImageIcon, Loader2, Trash2, Plus, X, Zap } from 'lucide-react'
...
export default function EditorContainer({ initialData }: EditorContainerProps) {
    ...
    const [html, setHtml] = useState(initialData?.contentHtml || '')
    const [visibility, setVisibility] = useState<'public' | 'private'>(initialData?.visibility || 'private')
    const [isSaving, setIsSaving] = useState(false)
    const [uploadMode, setUploadMode] = useState<'url' | 'upload'>('url')
    const [isUploading, setIsUploading] = useState(false)
    const [techs, setTechs] = useState<string[]>(initialData?.technologies || [])
    const [newTech, setNewTech] = useState('')
    const [resources, setResources] = useState<{ title: string, url: string }[]>(initialData?.resources || [])
    ...
    useEffect(() => {
        ...
        const timer = setInterval(() => {
            ...
            localStorage.setItem(`forge_draft_${initialData?.id || 'new'}`, JSON.stringify(draft))
            setLastSaved(new Date())
        }, 15000)

        return () => clearInterval(timer)
    }, [title, content, html, coverUrl, excerpt, techs, resources, initialData?.id])
    ...
    const handleSave = async () => {
        if (!title) {
            toast.error('A title is required before transmitting.')
            return
        }
        ...
        try {
            ...
            toast.success(initialData?.id ? 'Intelligence Briefing Updated' : 'Intelligence Briefing Transmitted')
            router.push('/blog/manage')
            router.refresh()
        } catch (error) {
            console.error(error)
            toast.error('Failed to save post. Check the console for details.')
        } finally {
            setIsSaving(false)
        }
    }
    ...
    return (
        <div className="h-full bg-[#050505] text-text-primary flex flex-col font-syne overflow-hidden relative">
            {/* Ambient Background */}
            ...
            <header className="h-24 border-b border-white/5 bg-black/40 backdrop-blur-xl px-8 flex items-center justify-between sticky top-0 z-[60]">
                <div className="flex items-center gap-8">
                    ...
                    >
                        ...
                        <div className="flex items-center gap-4 text-[9px] font-bold text-text-secondary uppercase tracking-widest mt-1">
                            <span className="text-primary/60">Stage: {initialData?.id ? 'Evolution' : 'Draft'}</span>
                            ...
                            {lastSaved && (
                                <>
                                    <span className="w-1 h-1 rounded-full bg-white/20" />
                                    <span className="text-primary/40 animate-pulse">Auto-saved {lastSaved.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' })}</span>
                                </>
                            )}
                        </div>
                    </motion.div>
                ...
                <div className="flex items-center gap-4">
                    <button
                        ...
                        className={cn(
                            "w-12 h-12 rounded-xl flex items-center justify-center border transition-all relative overflow-hidden group",
                            isAiOpen ? "bg-primary border-primary text-white shadow-lg shadow-primary/20" : "bg-white/5 border-white/5 text-text-secondary hover:text-white"
                        )}
                        title="Forge AI Assistant"
                    >
                        <Zap className={cn("w-5 h-5 relative z-10", isAiOpen && "fill-current animate-pulse")} />
                        {isAiOpen && (
                            <motion.div
                                layoutId="ai-glow"
                                className="absolute inset-0 bg-primary opacity-20 blur-xl"
                            />
                        )}
                    </button>
                    ...
                    >
                        ...
                        {initialData?.id ? 'Update Mission' : 'Transmit Briefing'}
                    </button>
                </div>
            </header>
                                    ...
                                    <div className="relative">
                                        {uploadMode === 'url' ? (
                                            ...
                                            <div className="relative group/upload">
                                                ...
                                                <div className={cn("bg-white/5 border border-white/5 rounded-2xl py-4 px-6 flex items-center justify-center gap-3 transition-all", isUploading ? <Loader2 className="w-4 h-4 animate-spin text-primary" /> : <Plus className="w-4 h-4 text-primary" />)}>
            ...
            <BlogAiAssistant
                ...
                title={title}
                content={html} // AI works better with clean text but HTML is okay if it's markdown or similar
                onApply={(type, value) => {
                    if (type === 'title') setTitle(value)
                    else if (type === 'excerpt') setExcerpt(value)
                    // Content apply usually needs to be careful not to overwrite everything,
                    // but for a 'polish' action it might be intended. 
                    // Tiptap can handle HTML input.
                    else if (type === 'content') {
                        setContent(value) // If it's HTML
                        // toast.success('AI intelligence merged.')
                    }
                    toast.success(`AI refinement applied: ${type}`)
                }}
            />
        </div>
    )
